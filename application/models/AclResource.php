...
class Application_Model_AclResource extends Application_Model_Mapper {
    ...
    public static function resourceExists($module = null, $controller = null, $action = null) {
        if (!$module || !$controller || !$action)
            throw new Exception("Error resourceExists(), the controller/action is empty");
        $result = self::getBySQLCondition(array('module=?' => $module, 'controller=?' => $controller, 'action=?' => $action));
    ...
    public function getCurrentRoleAllowedResources($role_id = null) {
        ...
        $sql = 'SELECT  A.module,A.controller,A.action FROM acl_to_roles ATR INNER JOIN acl A ON A.id=ATR.acl_id WHERE role_id=? ORDER BY A.controller';
...
//                $controller = $module.'/'.$value['controller'];
            }
    ...
    public function getAllResources() {
        $mapper = new self();
        $sql = 'SELECT module,controller,action FROM acl GROUP BY controller';
